...
namespace AElfChain.Common.Managers
{
    ...
    public class AElfKeyStore : IKeyStore
    {
        ...
        public async Task<ECKeyPair> CreateAccountKeyPairAsync(string password, ECKeyPair? keyPair)
        {
            var kp = keyPair ?? CryptoHelper.GenerateKeyPair();
            var res = await WriteKeyPairAsync(kp, password);
            return !res ? null : kp;
        }
        ...
        {
            var path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "aelf");
        ...
        public ECKeyPair CreateFakeAccountKeyPairAsync()
        {
