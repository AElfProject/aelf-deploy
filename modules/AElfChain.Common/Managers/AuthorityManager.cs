...
namespace AElfChain.Common.Managers
{
    public class AuthorityManager
    {
        ...
        public Address DeployContractWithAuthority(string caller, string contractName)
        {
            ...
            var code = contractReader.Read(contractName);
            code = GenerateUniqContractCode(code);

            return DeployContractWithAuthority(caller, code);
        }
        
        public Address DeployContractWithAuthorityAndContractOperation(string caller, byte[] code, ContractOperation? contractOperation)
        {
            return DeployContractWithAuthority(caller, code, contractOperation);
        }
        ...
        public void UpdatedContractWithAuthorityAndContractOperation(string caller, string contractAddress, byte[] code, ContractOperation? contractOperation)
        {
             UpdateContractWithAuthority(caller, contractAddress, code, contractOperation);
        }

        private Address DeployContractWithAuthority(string caller, byte[] code, ContractOperation? contractOperation = null)
        {
            ...
            if (!transactionResult.Equals(null))
            {
                ...
                var byteString3 =
                    ByteString.FromBase64(release.Logs.First(l => l.Name.Contains(nameof(ContractDeployed)))
                        .NonIndexed);
                var deployer =  ContractDeployed.Parser.ParseFrom(byteString3).Deployer;
                var deployAddress = ContractDeployed.Parser.ParseFrom(byteString3).Address;
                Logger.Info(deployer != null
                    ? $"Contract deploy passed authority, contract address: {deployAddress}, deployer: {deployer}"
                    : $"Contract deploy passed authority, contract address: {deployAddress}");
        ...
        private void UpdateContractWithAuthority(string caller, string address, byte[] code, ContractOperation? contractOperation = null)
        {
...
//            create association organization
        ...
        private void CheckBpBalance(string caller)
        {
            ...
            foreach (var bp in bps)
            {
                ...
                if (balance < 1000_00000000)
                    _token.TransferBalance(caller, bp, 1000_00000000, primaryToken);
                balance = _token.GetUserBalance(bp, primaryToken);
                Logger.Info($"{bp} balance: {balance}");
            }
        }
        ...
        public byte[] GenerateUniqContractCode(byte[] code)
        {
            while (true)
            {
