...
namespace AElfChain.Common.Contracts
{
    ...
    public class TokenConverterContract : BaseContract<TokenConverterMethod>
    {
        ...
        public TransactionResultDto Sell(string seller,string symbol,long amount)
        {
            var tester = GetNewTester(seller);
            var result = tester.ExecuteMethodWithResult(TokenConverterMethod.Sell, new SellInput()
            {
                Symbol = symbol,
                Amount = amount
            });
