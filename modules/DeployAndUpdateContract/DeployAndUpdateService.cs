...
public class DeployAndUpdateService
{
    ...
    public void UpdateContracts(bool isApproval, UpdateInfo updateInfo, string file, AuthorInfo author, string salt = "")
    {
        ...
        if (isApproval || updateInfo.isSystemContract)
        {
            ...
            ApproveByMiner(contractProposalInfo.ProposalId);
            var releaseCodeCheckInput = ReleaseApprove(contractProposalInfo);
            ReleaseCodeCheck(releaseCodeCheckInput, "update");
        }
        else
        {
            var input = new ContractUpdateInput
    ...
    private void ReleaseCodeCheck(ReleaseContractInput input, string type)
    {
        ...
        _logger?.Info("======== Check contract Info ========");
        var contractInfo =
            _service.GenesisService.CallViewMethod<ContractInfo>(GenesisMethod.GetContractInfo,
                changeAddress);
